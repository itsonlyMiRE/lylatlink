...
STAGE="$ROOT/dist/.macos-stage-$ARCH"
ZIP_ROOT="$STAGE/LylatLink-macos-$ARCH"

rm -rf "$APP" "$HELPER_APP" "$ZIP" "$STAGE"
mkdir -p "$MACOS" "$RESOURCES" "$FRAMEWORKS" "$HELPER_MACOS" "$HELPER_RESOURCES" "$ROOT/dist" "$STAGE"
